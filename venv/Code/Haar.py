...
accumulator_kernel = 30
gaussian_kernel = 3
min_radius = 23
max_radius = 24
output_file = 'result.jpg'
input_file_name = 'p2.jpg'
...
# Here is where a min radius could be implemented
...
def construct_accumulator_array(edges, width, height, max_radius):
    ...
    for radius in range(23, max_radius):
        for i in range(0,len(edges[0])): # Maybe add Gradient Direction??
            x=edges[0][i]
            y=edges[1][i]
...
def main(filename, intensity_threshold, accumulator_kernel, gaussian_kernel, max_detection_radius, output_file):
    ...
    #get_max_possible_radius(edges, max_detection_radius)
...
main(input_file_name, intensity_threshold, accumulator_kernel, gaussian_kernel, 100, output_file)